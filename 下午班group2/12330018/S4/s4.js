...
function addLiClick(i){
  ...
  if(i>=mylist.length)
    return;
  mylist[index].disabled = true;
  beforeClick(mylist,index);
  sendAjax(mylist[index],index, function(data) {
    mylist[index].querySelector('span').innerText = data;
    allowOtherLi(mylist,index);
    bigIsReady();
    setTimeout("addLiClick("+(i+1)+")",800);
  });
}

//random 数组
function myrandom(length){
  var myarr = new Array;
